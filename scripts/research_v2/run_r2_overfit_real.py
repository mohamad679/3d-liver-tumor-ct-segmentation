#!/usr/bin/env python3
"""Run the locked R2 timing calibration or primary few-real-train-patient overfit experiment."""
...
def _require_cuda_contract(torch: Any) -> dict[str, Any]:
    if not bool(torch.cuda.is_available()):
        raise RuntimeError("R2 primary/calibration run requires CUDA; CPU fallback is forbidden")
    if int(torch.cuda.device_count()) != 2:
        raise RuntimeError("R2 requires the registered two-device Kaggle T4 environment")
...
def _verify_preflight(path: Path) -> dict[str, Any]:
    if sha256_file(path) != EXPECTED_PREFLIGHT_SHA256:
        raise RuntimeError("R2 preflight evidence SHA-256 differs from the verified artifact")
    evidence = _load_json(path)
    if evidence.get("status") != "PASS" or evidence.get("main_training_started") is not False:
        raise RuntimeError("R2 preflight evidence is not a clean PASS before main training")
    environment = evidence.get("environment")
    gpu = evidence.get("gpu")
    step = evidence.get("forward_backward_optimizer_step")
    array_access = evidence.get("array_access")
    if not isinstance(environment, dict) or environment.get("uv_lock_sha256_verified") is not True:
    ...
    if gpu.get("training_device") != "cuda:0" or gpu.get("cpu_fallback_allowed") is not False:
        raise RuntimeError("R2 preflight device contract is inconsistent")
    if not isinstance(step, dict) or step.get("status") != "PASS":
        raise RuntimeError("R2 preflight optimizer step did not PASS")
    if not all(step.get(field) is True for field in ("finite_logits", "finite_loss", "finite_gradients")):
        raise RuntimeError("R2 preflight finite checks did not all PASS")
    if not isinstance(array_access, dict):
        raise RuntimeError("R2 preflight array-access evidence is missing")
    for field in ("validation_arrays_opened", "internal_test_arrays_opened", "external_arrays_opened"):
        if int(array_access.get(field, -1)) != 0:
            raise RuntimeError(f"R2 preflight leakage boundary failed: {field}")
    return evidence


def _validate_locked_config(config: dict[str, Any]) -> None:
    ...
    if not isinstance(training, dict) or not isinstance(inference, dict) or not isinstance(budget, dict):
        raise RuntimeError("R2 config is missing training/inference/budget sections")
    required_training = {
        "seed": R2_SEED,
    ...
    for key, expected in required_training.items():
        if training.get(key) != expected:
            raise RuntimeError(f"R2 locked training field changed: {key}")
    if training.get("target_spacing_mm") != list(R2_TARGET_SPACING_MM):
        raise RuntimeError("R2 target spacing changed")
    if inference.get("sliding_window_roi_size") != list(R2_PATCH_SIZE):
        raise RuntimeError("R2 sliding-window ROI changed")
    if inference.get("sliding_window_overlap") != 0.5 or inference.get("threshold") != R2_THRESHOLD:
        raise RuntimeError("R2 inference overlap or threshold changed")
    ...
    if not isinstance(calibration, dict) or not isinstance(primary, dict):
        raise RuntimeError("R2 budget sections are missing")
...
def _verify_selected_files(data_root: Path) -> None:
    ...
    for spec in R2_SELECTED_CASES:
        image_path = data_root / spec.anonymous_case_id / "image.nii.gz"
        label_path = data_root / spec.anonymous_case_id / "label.nii.gz"
        expected_paths.update({image_path.resolve(), label_path.resolve()})
        if sha256_file(image_path) != spec.image_sha256:
            raise RuntimeError(f"R2 image SHA-256 mismatch: {spec.anonymous_case_id}")
        if sha256_file(label_path) != spec.label_sha256:
            raise RuntimeError(f"R2 label SHA-256 mismatch: {spec.anonymous_case_id}")
    observed = {
        path.resolve()
    ...
    if observed != expected_paths:
        raise RuntimeError("R2 data root must contain exactly the eight selected train NIfTI files")


def _prepare_case(data_root: Path, spec: Any) -> PreparedCase:
    image_nifti = nib.load(str(data_root / spec.anonymous_case_id / "image.nii.gz"))
    label_nifti = nib.load(str(data_root / spec.anonymous_case_id / "label.nii.gz"))
    ...
    if image.ndim != 3 or label.ndim != 3 or image.shape != label.shape:
        raise RuntimeError(f"R2 source image/label shape mismatch: {spec.anonymous_case_id}")
    ...
    if float(np.max(np.abs(image_affine - label_affine))) > 1e-4:
        raise RuntimeError(f"R2 source image/label affine mismatch: {spec.anonymous_case_id}")
    native_tumor = r2_binary_tumor_target(label)
    if int(np.count_nonzero(native_tumor)) != int(spec.tumor_voxel_count):
        raise RuntimeError(f"R2 native tumor voxel count mismatch: {spec.anonymous_case_id}")
    ...
    if spec.tumor_voxel_count > 0 and not bool(np.any(resampled_tumor)):
        raise RuntimeError(f"R2 positive tumor vanished after nearest resampling: {spec.anonymous_case_id}")
    if spec.tumor_voxel_count == 0 and bool(np.any(resampled_tumor)):
        raise RuntimeError(f"R2 empty case became positive after resampling: {spec.anonymous_case_id}")
    return PreparedCase(
...
def _training_patch(cases: list[PreparedCase], *, step: int, rng: np.random.Generator) -> tuple[np.ndarray, np.ndarray, dict[str, Any]]:
    ...
    if role == "positive" and tumor_voxels <= 0:
        raise RuntimeError("R2 positive patch does not contain tumor voxels")
    if role == "empty" and tumor_voxels != 0:
        raise RuntimeError("R2 empty-case patch unexpectedly contains tumor voxels")
    metadata = {
...
def _optimizer_step(*, torch: Any, model: Any, optimizer: Any, scaler: Any, loss_function: Any, image_patch: np.ndarray, target_patch: np.ndarray, device: Any) -> dict[str, Any]:
    model.train()
    ...
    if not bool(torch.isfinite(logits).all().item()) or not bool(torch.isfinite(loss).item()):
        raise RuntimeError("R2 training step produced non-finite logits or loss")
    ...
    for parameter in model.parameters():
        ...
        if not bool(torch.isfinite(parameter.grad).all().item()):
            raise RuntimeError("R2 training step produced non-finite gradients")
    if gradient_tensor_count == 0:
        raise RuntimeError("R2 training step produced no gradients")
    ...
    probability_cpu = probability.detach().cpu().numpy()[0]
    patch_prediction = probability_cpu >= R2_THRESHOLD
    patch_dice = binary_dice(np.asarray(target_patch, dtype=bool), patch_prediction)
    return {
        "loss": float(loss.detach().float().cpu().item()),
        "patch_dice": patch_dice,
        "probability_min": float(probability_cpu.min()),
        "probability_max": float(probability_cpu.max()),
        "probability_mean": float(probability_cpu.mean()),
        "finite_logits": True,
        "finite_loss": True,
...
def _sliding_window_probability(*, torch: Any, monai: Any, model: Any, case: PreparedCase, device: Any) -> tuple[np.ndarray, float]:
    model.eval()
    input_cpu = torch.from_numpy(case.normalized_image[None, None]).to(dtype=torch.float32)
    started = time.perf_counter()
    with torch.no_grad():
    ...
    if not bool(np.isfinite(probability).all()):
        raise RuntimeError(f"R2 full-volume probability is non-finite: {case.anonymous_case_id}")
    return np.asarray(probability, dtype=np.float32), float(elapsed)


def _diagnostic_patch_metrics(*, torch: Any, model: Any, case: PreparedCase, device: Any, index: int) -> dict[str, Any]:
    if bool(np.any(case.resampled_tumor)):
        center = choose_positive_center(case.resampled_tumor, seed=R2_SEED + index)
    else:
        center = tuple(int(value // 2) for value in case.resampled_tumor.shape)
    image_patch = extract_centered_patch(case.normalized_image, center=center, patch_size=R2_PATCH_SIZE, pad_value=-1.0).astype(np.float32, copy=False)
    target_patch = extract_centered_patch(case.resampled_tumor, center=center, patch_size=R2_PATCH_SIZE, pad_value=False).astype(bool, copy=False)
    model.eval()
    with torch.no_grad():
        logits = model(torch.from_numpy(image_patch[None, None]).to(device=device, dtype=torch.float32))
    ...
    return {
        ...
        "patch_dice": binary_dice(np.asarray(target_patch, dtype=bool), prediction),
...
def _evaluate_case(*, torch: Any, monai: Any, model: Any, case: PreparedCase, device: Any, index: int, reference_dir: Path | None) -> dict[str, Any]:
    patch_metrics = _diagnostic_patch_metrics(torch=torch, model=model, case=case, device=device, index=index)
    probability_resampled, inference_seconds = _sliding_window_probability(torch=torch, monai=monai, model=model, case=case, device=device)
    ...
    if reference_dir is not None:
        ...
        np.save(case_dir / "native_probability.npy", probability_native, allow_pickle=False)
        np.save(case_dir / "native_binary.npy", prediction_native.astype(np.uint8), allow_pickle=False)
    return {
        "anonymous_case_id": case.anonymous_case_id,
        "role": case.role,
        "patch": patch_metrics,
        "native_full_volume": {
            ...
            "ground_truth_tumor_voxels": int(metrics.ground_truth_foreground_voxels),
            ...
            "false_positive_lesion_count": int(metrics.false_positive_lesion_count),
            "false_negative_lesion_count": int(metrics.false_negative_lesion_count),
...
def _evaluate_all(*, torch: Any, monai: Any, model: Any, cases: list[PreparedCase], device: Any, step: int, reference_dir: Path | None) -> dict[str, Any]:
    started = time.perf_counter()
    records = [
        _evaluate_case(
            torch=torch,
...
def _checkpoint_payload(*, torch: Any, model: Any, optimizer: Any, scaler: Any, rng: np.random.Generator, step: int, config_sha256: str) -> dict[str, Any]:
    return {
...
def _run_calibration(*, torch: Any, monai: Any, cases: list[PreparedCase], output_dir: Path, config_sha256: str, gpu_evidence: dict[str, Any], preflight_sha256: str) -> int:
    ...
    loss_function = _make_loss(monai)
    torch.cuda.empty_cache()
    torch.cuda.reset_peak_memory_stats(device)
    records: list[dict[str, Any]] = []
    for step in range(1, R2_CALIBRATION_MAX_STEPS + 1):
        image_patch, target_patch, sampling = _training_patch(cases, step=step, rng=rng)
    ...
    if not bool(np.isfinite(probe_probability).all()):
        raise RuntimeError("R2 calibration full-volume inference probe is non-finite")
    ...
    summary = {
        ...
        "gpu": gpu_evidence,
        "mean_step_time_seconds": float(statistics.fmean(step_times)),
        "max_step_time_seconds": float(max(step_times)),
        "projected_2500_optimizer_step_hours_excluding_evaluations": float(statistics.fmean(step_times) * R2_PRIMARY_MAX_STEPS / 3600.0),
        ...
        "positive_patch_count": sum(record["sampling_role"] == "positive" for record in records),
        ...
        "interpretation": "Transient real selected-train calibration for timing/memory only. The primary R2 run must restart from the locked seed and random initialization.",
    }
    _write_json(output_dir / "r2_calibration_summary.json", summary)
    with (output_dir / "r2_calibration_steps.jsonl").open("w", encoding="utf-8") as handle:
...
def _run_primary(*, torch: Any, monai: Any, cases: list[PreparedCase], output_dir: Path, config_sha256: str, gpu_evidence: dict[str, Any], preflight_sha256: str, calibration_path: Path) -> int:
    calibration = _load_json(calibration_path)
    if calibration.get("status") != "PASS" or calibration.get("mode") != "timing_memory_calibration":
        raise RuntimeError("R2 primary run requires PASS calibration evidence")
    if calibration.get("main_training_started") is not False or calibration.get("training_warm_start_allowed") is not False:
        raise RuntimeError("R2 calibration evidence violates fresh-start policy")
    device = torch.device("cuda:0")
    torch.manual_seed(R2_SEED)
    torch.cuda.manual_seed_all(R2_SEED)
    rng = np.random.default_rng(R2_SEED)
    ...
    with log_path.open("w", encoding="utf-8") as log_handle:
        for step in range(1, R2_PRIMARY_MAX_STEPS + 1):
            ...
            record = {
                ...
                **sampling,
                **metrics,
                "positive_patch_ratio_cumulative": float(positive_patches / step),
            ...
            if step % R2_PATCH_LOG_EVERY_STEPS == 0:
                print(
                    ...
                    f"positive_ratio={record['positive_patch_ratio_cumulative']:.4f}"
                )
            if step % R2_FULL_VOLUME_EVAL_EVERY_STEPS == 0:
                save_reference = step == R2_PRIMARY_MAX_STEPS
                reference_dir = output_dir / "private_reload_reference" if save_reference else None
    ...
    if final_step <= 0:
        raise RuntimeError("R2 primary run ended before completing any optimizer step")
    if not evaluations or int(evaluations[-1]["step"]) != final_step:
    ...
    elif not (output_dir / "private_reload_reference").exists():
        # The final scheduled evaluation occurred at max steps but must also persist reload references.
        evaluations[-1] = _evaluate_all(
            torch=torch,
            monai=monai,
            model=model,
            cases=cases,
    ...
    _write_json(output_dir / "r2_evaluations.json", {"schema_version": "research_v2_r2_evaluations.v1", "evaluations": evaluations})
    final_cases = evaluations[-1]["cases"]
    positive_dice = [
        float(record["native_full_volume"]["tumor_dice"])
        for record in final_cases
        if str(record["role"]).startswith("positive_")
    ]
    empty_records = [record for record in final_cases if record["role"] == "empty_lexicographic_first"]
    if len(positive_dice) != 3 or len(empty_records) != 1:
        raise RuntimeError("R2 final evaluation case-role cardinality is invalid")
    empty_prediction_voxels = int(empty_records[0]["native_full_volume"]["predicted_tumor_voxels"])
    ...
    summary = {
        ...
        "gpu_wall_seconds": float(wall_seconds),
        ...
        "all_positive_native_full_volume_dice_at_least_0_95": all(value >= 0.95 for value in positive_dice),
        ...
        "interpretation": "Diagnostic training-set overfit run only; these results are not evidence of unseen-patient performance.",
    }
...
def main() -> int:
    ...
    parser.add_argument("--mode", choices=("calibration", "primary"), required=True)
    ...
    if not data_root.is_dir():
        raise RuntimeError(f"R2 selected train data root does not exist: {data_root}")
    if output_dir.exists():
        raise RuntimeError(f"refusing to overwrite existing R2 run directory: {output_dir}")
    output_dir.mkdir(parents=True)

    lock_path = repo_root / LOCK_RELATIVE_PATH
    if sha256_file(lock_path) != EXPECTED_LOCK_SHA256:
        raise RuntimeError("R2 uv.lock differs from the R0 frozen Linux CUDA lock")
    ...
    gpu_evidence = _require_cuda_contract(torch)
    ...
    try:
        ...
        if args.calibration_evidence is None:
            raise RuntimeError("R2 primary mode requires --calibration-evidence")
        return _run_primary(
            torch=torch,
            monai=monai,
            cases=cases,
        ...
        failure = {
            ...
            "error": str(exc),
            "main_training_started": args.mode == "primary",
            "threshold_changed": False,
            "gate_threshold_changed": False,
            "validation_arrays_opened": 0,
            "internal_test_arrays_opened": 0,
