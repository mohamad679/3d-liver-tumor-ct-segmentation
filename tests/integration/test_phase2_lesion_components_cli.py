"""Integration tests for the Phase 2 lesion-components CLI."""
    ...
    *,
    ...
    cases = [
        _write_case(
            root,
            index=1,
            image_data=cast(NiftiArray, np.zeros(label.shape, dtype=np.float32)),
    ...
    if mixed_failure:
        bad_label = np.full((3, 3, 3), 2, dtype=np.uint8)
        cases.append(
            _write_case(
                root,
                index=2,
                image_data=cast(NiftiArray, np.ones(bad_label.shape, dtype=np.float32)),
                label_data=cast(NiftiArray, bad_label),
                affine=matching_affine,
                write_nifti_file=write_nifti_file,
            )
        )
    ...
    return root, manifest_path, geometry_path, split_path


def _lesion_args(root: Path, manifest_path: Path, geometry_path: Path, output: Path) -> list[str]:
    return [
        "summarize-lesions",
        "--manifest",
        str(manifest_path),
...
def test_successful_synthetic_lesion_summary_cli(
    tmp_path: Path,
    write_nifti_file: WriteNiftiFile,
    matching_affine: AffineArray,
    valid_ct_image: NiftiArray,
) -> None:
    root, manifest_path, geometry_path, _split_path = _write_inputs(
        tmp_path,
        write_nifti_file,
        matching_affine,
        valid_ct_image,
    ...
    result = _invoke(*_lesion_args(root, manifest_path, geometry_path, output))

    assert result.exit_code == 0
    assert "lesion summary completed" in result.output
    ...
    assert "connectivity: 26" in result.output
    ...
    assert artifact.lesion_artifact_hash == hash_lesion_components(artifact)
    assert artifact.case_records[0].lesion_count == 1


def test_mixed_skipped_case_cli_exits_zero_without_identifiers(
    tmp_path: Path,
    write_nifti_file: WriteNiftiFile,
    matching_affine: AffineArray,
    valid_ct_image: NiftiArray,
...
def test_repeated_cli_outputs_are_byte_identical(
    ...
    first_output = tmp_path / "first" / "lesions.json"
    second_output = tmp_path / "second" / "lesions.json"

    first = _invoke(*_lesion_args(root, manifest_path, geometry_path, first_output))
    second = _invoke(*_lesion_args(root, manifest_path, geometry_path, second_output))
...
def test_lesion_cli_structural_failures_are_bounded(
    tmp_path: Path,
    write_nifti_file: WriteNiftiFile,
    matching_affine: AffineArray,
    valid_ct_image: NiftiArray,
    ...
    tampered = replace(geometry, qa_artifact_hash="f" * 64)
    geometry_path.write_text(phase2_artifact_to_json(tampered), encoding="utf-8")
    altered = _invoke(*_lesion_args(root, manifest_path, geometry_path, output))
    assert altered.exit_code != 0
    assert "Phase 2 lesion-components error: " in _error_text(altered)
    ...
    output = tmp_path / "changed-out" / "lesions.json"
    (root / "labels/case-0001.nii.gz").write_bytes(b"changed")
    changed = _invoke(*_lesion_args(root, manifest_path, geometry_path, output))
    assert changed.exit_code != 0
    assert "Traceback" not in _error_text(changed)
    assert str(root) not in _error_text(changed)
    assert "labels/" not in _error_text(changed)
    ...
    output = tmp_path / "existing-output" / "lesions.json"
    output.parent.mkdir(parents=True)
    output.write_text("occupied\n", encoding="utf-8")
    existing = _invoke(*_lesion_args(root, manifest_path, geometry_path, output))
