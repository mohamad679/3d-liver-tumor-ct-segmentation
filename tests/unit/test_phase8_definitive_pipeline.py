...
from protoem_ct.external.definitive_pipeline import (
    ...
    REQUIRED_BLOCKS_DOWN,
    REQUIRED_BLOCKS_UP,
    REQUIRED_DROPOUT_PROB,
    REQUIRED_IN_CHANNELS,
    REQUIRED_INIT_FILTERS,
    REQUIRED_OUT_CHANNELS,
    REQUIRED_SPATIAL_DIMS,
    REQUIRED_UPSAMPLE_MODE,
...
def test_build_definitive_config_v1_has_locked_architecture_fields() -> None:
    config = build_definitive_config_v1()
    assert config.spatial_dims == 3
    assert config.in_channels == 1
    assert config.out_channels == 2
    assert config.init_filters == 8
    assert config.blocks_down == (1, 1, 1)
    assert config.blocks_up == (1, 1)
    assert config.dropout_prob is None
    assert config.upsample_mode == "deconv"
    assert config.spatial_dims == REQUIRED_SPATIAL_DIMS
    assert config.in_channels == REQUIRED_IN_CHANNELS
    assert config.out_channels == REQUIRED_OUT_CHANNELS
    assert config.init_filters == REQUIRED_INIT_FILTERS
    assert config.blocks_down == REQUIRED_BLOCKS_DOWN
    assert config.blocks_up == REQUIRED_BLOCKS_UP
    assert config.dropout_prob == REQUIRED_DROPOUT_PROB
    assert config.upsample_mode == REQUIRED_UPSAMPLE_MODE
...
def test_tampered_architecture_field_fails_closed(field_name: str, bad_value: object) -> None:
    config = build_definitive_config_v1()
    payload = {f.name: getattr(config, f.name) for f in dataclasses.fields(config)}
    payload[field_name] = bad_value
    with pytest.raises(Phase8DefinitivePipelineConfigError):
...
# ---------------------------------------------------------------------------
# G. Memory-safe training path (Phase8DefinitiveTrainCaseReference / loader)
# ---------------------------------------------------------------------------
...
class _LazyCaseLoader:
    ...
    Mirrors the shape a real caller-supplied loader would have: it never
    holds full-volume arrays for cases it has not been asked for, and it
    records every ``case_id`` it is invoked with (in order) so tests can
    verify which cases were actually requested and how many times.
    """
...
@requires_baseline_environment
def test_build_definitive_segresnet_model_uses_config_architecture_fields() -> None:
    import monai
    import torch
    ...
    dummy_input = torch.zeros((1, config.in_channels, 64, 64, 32), dtype=torch.float32)
    with torch.no_grad():
        output = model(dummy_input)

    assert output.shape[0] == 1
    assert output.shape[1] == config.out_channels
    assert tuple(output.shape[2:]) == tuple(dummy_input.shape[2:])


@requires_baseline_environment
def test_run_definitive_training_step_produces_valid_patch_composition() -> None:
    import monai
    import torch

    from protoem_ct.external.definitive_pipeline import (
        _build_definitive_segresnet_model,
        _run_definitive_training_step,
    )

    config = build_definitive_config_v1()
    positive_case = _make_synthetic_train_case("case_p", positive=True)
    negative_case = _make_synthetic_train_case("case_n", positive=False)

    # The shared step helper internally calls sample_foreground_aware_patches
    # with positive_count=1/negative_count=0 for the positive case and
    # positive_count=0/negative_count=1 for the negative case; verify that
    # composition directly satisfies the same foreground/background
    # invariants asserted for sample_foreground_aware_patches itself.
    ...
    assert bool(np.any(positive_patches[0].label_patch))
    assert not bool(np.any(negative_patches[0].label_patch))
...
@requires_baseline_environment
def test_run_definitive_training_from_references_succeeds_with_small_pool() -> None:
    config = build_definitive_config_v1()
    case_ids = ["case_pos", "case_neg_a", "case_neg_b"]
    case_references = [Phase8DefinitiveTrainCaseReference(case_id=cid) for cid in case_ids]
    for reference in case_references:
        assert not hasattr(reference, "image")
        assert not hasattr(reference, "label_binary")

    loader = _LazyCaseLoader(positive_ids={"case_pos"})

    result = run_definitive_training_from_references(
        case_references,
...
@requires_baseline_environment
def test_run_definitive_training_from_references_rejects_unknown_positive_case_id() -> None:
    config = build_definitive_config_v1()
    case_references = [Phase8DefinitiveTrainCaseReference(case_id="case_a")]
    loader = _LazyCaseLoader(positive_ids={"case_a"})
    with pytest.raises(Phase8DefinitivePipelineRuntimeError):
        run_definitive_training_from_references(
            case_references,
            positive_case_ids={"case_missing"},
            loader=loader,
            config=config,
            max_steps=1,
        )
...
@requires_baseline_environment
def test_run_definitive_training_from_references_rejects_no_positive_case_ids() -> None:
    config = build_definitive_config_v1()
    case_references = [Phase8DefinitiveTrainCaseReference(case_id="case_a")]
    loader = _LazyCaseLoader(positive_ids=set())
    with pytest.raises(Phase8DefinitivePipelineRuntimeError):
        run_definitive_training_from_references(
            case_references,
            positive_case_ids=set(),
            loader=loader,
            config=config,
            max_steps=1,
        )
...
@requires_baseline_environment
def test_run_definitive_training_from_references_loader_called_only_for_required_cases() -> None:
    config = build_definitive_config_v1()
    case_ids = [f"case_{i}" for i in range(5)]
    case_references = [Phase8DefinitiveTrainCaseReference(case_id=cid) for cid in case_ids]
    loader = _LazyCaseLoader(positive_ids={"case_0"})

    max_steps = 3
    result = run_definitive_training_from_references(
        case_references,
        positive_case_ids={"case_0"},
        loader=loader,
        config=config,
        max_steps=max_steps,
    )
    ...
    assert all(case_id in known_ids for case_id in loader.calls)
    # Not every case reference is loaded on every step: at most two cases
    # (positive + negative) are loaded per step, far fewer than the full
    # pool of 5.
    assert len(loader.calls) < len(case_references) * max_steps


@requires_baseline_environment
def test_run_definitive_training_from_references_large_pool_uses_cheap_references() -> None:
    config = build_definitive_config_v1()
    case_ids = [f"case_{i:03d}" for i in range(80)]
    case_references = [Phase8DefinitiveTrainCaseReference(case_id=cid) for cid in case_ids]
    loader = _LazyCaseLoader(positive_ids={"case_000"})

    max_steps = 2
    result = run_definitive_training_from_references(
        case_references,
        positive_case_ids={"case_000"},
        loader=loader,
        config=config,
        max_steps=max_steps,
    )
    ...
    assert result.all_steps_finite is True
    # Building 80 cheap case_id-only references must never require
    # constructing 80 full Phase8DefinitiveTrainCase objects; the lazy
    # loader only constructs at most 2 full cases per step.
    assert loader.construction_count <= 2 * max_steps


@requires_baseline_environment
def test_run_definitive_training_from_references_is_deterministic_across_runs() -> None:
    config = build_definitive_config_v1()
    case_ids = [f"case_{i}" for i in range(5)]
    case_references = [Phase8DefinitiveTrainCaseReference(case_id=cid) for cid in case_ids]

    loader_a = _LazyCaseLoader(positive_ids={"case_0"})
    loader_b = _LazyCaseLoader(positive_ids={"case_0"})

    result_a = run_definitive_training_from_references(
        case_references,
        positive_case_ids={"case_0"},
        loader=loader_a,
        config=config,
    ...
    result_b = run_definitive_training_from_references(
        case_references,
        positive_case_ids={"case_0"},
        loader=loader_b,
        config=config,
    ...
    assert result_a.all_steps_finite == result_b.all_steps_finite
    assert [s.loss_value for s in result_a.step_results] == [
        s.loss_value for s in result_b.step_results
    ]


@requires_baseline_environment
def test_run_definitive_training_from_references_loads_once_when_positive_equals_negative() -> None:
    config = build_definitive_config_v1()
    # A single-case pool guarantees the deterministically chosen negative
    # case index (always 0) coincides with the single positive case.
    ...
    assert result.all_steps_finite is True
    assert len(result.step_results) == max_steps
    # Exactly one loader call per step, not two, when positive == negative.
    assert loader.calls == ["only_case"] * max_steps
    assert loader.construction_count == max_steps


@requires_baseline_environment
def test_run_definitive_training_from_references_reloads_case_every_step_no_persistent_cache() -> (
    None
):
    config = build_definitive_config_v1()
    case_references = [
        Phase8DefinitiveTrainCaseReference(case_id="case_a"),
        Phase8DefinitiveTrainCaseReference(case_id="case_b"),
    ]
    loader = _LazyCaseLoader(positive_ids={"case_a"})
    ...
    assert result.all_steps_finite is True
    assert len(result.step_results) == max_steps
    # "case_a" is the positive case, so it is requested from the loader on
    # every single step. If a persistent cross-step cache existed, the
    # loader would have been invoked for it only once overall instead of
    # once per step.
    assert loader.calls.count("case_a") == max_steps


# ---------------------------------------------------------------------------
# H. Static scan: no external-dataset / real-filesystem literals
# ---------------------------------------------------------------------------
